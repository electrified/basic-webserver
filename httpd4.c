...
// Function to serve a file from disk
int serve_file_from_disk(const char *filepath, char *response, size_t response_size) {
    ...
    // Read the file contents into the response buffer after the headers
    ssize_t bytes_read = read(file_fd, response + strlen(response), response_size - strlen(response) - 1);
    if (bytes_read == -1) {
    ...
    while (1) {
        ...
        if (strcmp(method, "POST") == 0 && strcmp(resource, "/submit") == 0) {
            ...
            if (html_content) {
                ...
                if (serve_file_from_disk(filepath, response, BUFFER_SIZE) == -1) {
